...
import com.jam.dto.CreateStoryDTO;
...
public class WriterService {
	 ...
	 */
	@Transactional
	public void createBook(CreateBookDTO bookDTO, Integer principalId) {
	    // 책 생성 결과를 저장할 변수
	    int result = 0;

	    try {
	        // Book 객체를 생성하고 데이터베이스에 삽입
	        result = bookRepository.insertBook(bookDTO.createBook(principalId));

	        // 생성된 Book ID를 가져옴
	        int bookId = bookDTO.getBookId();

	        // 카테고리, 장르, 태그 정보도 삽입
	        bookRepository.insertBookCategories(bookDTO.getCategoryIds(), bookId);
	        bookRepository.insertBookGenres(bookDTO.getGenreIds(), bookId);
	        bookRepository.insertBookTags(bookDTO.getTagIds(), bookId);
	 ...
	 */
	@Transactional
	public void insertStory(CreateStoryDTO storyDTO, Integer bookId, Integer principalId) {
		int result = 0;
		try {
	 ...
	 * @return
	 */
	public Story outputStoryContentByStoryIdAndNumber(Integer storyId, Integer number) {
		Story story = new Story();
		try {
			story = storyRepository.outputStoryContentByNumber(storyId, number);
		} catch (Exception e) {
			// TODO - 오류 처리
		}
		return story;

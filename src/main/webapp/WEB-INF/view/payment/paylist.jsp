							...
							<c:forEach var="payment" items="${payList}">
								<tr>
									...
									<td>
										<c:choose>
											<c:when
												test="${payment.status == 'PENDING' && fn:trim(payment.event) == 'N'}">
												<!-- status가 'PENDING'일 때만 환불 버튼을 보여줌 -->
												<form id="refundForm_${payment.paymentKey}" action="/pay/manager"
													method="POST">
													...
													<input type="hidden" id="refundReasonHidden_${payment.paymentKey}"
														name="refundReason" value="${payment.refundReason}">
													<button type="button" id="refundButton_${payment.paymentKey}"
														onclick="setRefundReasonAndSubmit('${payment.paymentKey}', '${payment.deposit}')">환불</button>
												</form>
											</c:when>
											<c:otherwise>
								...
								<table>
									...
									<tr>
										...
										<c:choose>
											...
											<c:when test="${funding.isClosed eq 'Y'}">
												<td><button type="button" disabled>펀딩 취소</button></td>
											</c:when>

											<c:otherwise>
												<form method="post" action="/funding/cancelFunding">
													<input type="hidden" name="fundingId" value="${funding.fundingId}">
													<input type="hidden" name="totalAmount"
														value="${funding.rewardPoint}">
													<td><button type="submit" id="cancelFunding">펀딩 취소</button></td>
												</form>
											</c:otherwise>
										</c:choose>
									</tr>

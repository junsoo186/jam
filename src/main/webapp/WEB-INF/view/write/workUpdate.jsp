...
<body>
	...
	<form action="workUpdate" method="post" onsubmit="appendCustomTag()">
		...
		<select id="categoryName" name="categoryName" required>
			...
			<option value="비소설" ${bookDetail.categoryName=='비소설' ? 'selected' : ''}>비소설</option>
			<option value="역사" ${bookDetail.categoryName=='역사' ? 'selected' : ''}>역사</option>
			<option value="과학" ${bookDetail.categoryName=='과학' ? 'selected' : ''}>과학</option>
		</select>
		<br> 
		<br> 
		<label for="genreName">장르 이름:</label> 
		<select id="genreName" name="genreNames">
			<option value="" ${bookDetail.genreName=='' ? 'selected' : ''}></option>
			<option value="판타지" ${bookDetail.genreName=='판타지' ? 'selected' : ''}>판타지</option>
			<option value="추리" ${bookDetail.genreName=='추리' ? 'selected' : ''}>추리</option>
			<option value="로맨스" ${bookDetail.genreName=='로맨스' ? 'selected' : ''}>로맨스</option>

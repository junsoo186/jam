			...
			<div class="form-section">
				...
				<div class="left-section">
					...
					<div class="book-cover">
						...
						<!-- 기존 이미지 표시 -->
						<div>
							<img id="currentCoverImage" src="${bookDetail.bookCoverImage}" alt="현재 책 커버 이미지" style="max-width: 100%; height: auto;">
						</div>

						<!-- 새로운 이미지 업로드 입력 -->
						<input type="file" id="bookCover" name="bookCover" style="display: none" onchange="previewNewImage(event)">
				...
				<div class="right-section">
					...
					<label for="title">제목:</label> <input type="text" id="title" name="title" value="${bookDetail.title}" required /> <label for="authorComment">저자 코멘트:</label>
					...
					<!-- 기본값을 "추리"로 설정 -->
					<br>
					<br> <label for="serialDay">연재 요일:</label>
					...
					<div id="tagList" class="tag-input">
						<c:forEach items="${selectTags}" var="tag">
							<div class="tag-item">
								<span class="tag-name">${tag.tagName}</span> <span class="remove-tag" onclick="removeTag('${tag.tagName}', this)">x</span> <input type="hidden" name="customTag"
									value="${tag.tagName}">
							</div>
						</c:forEach>
					</div>
					...
					<div class="tag-input">
						<input type="text" id="customTag" name="customTag" onkeydown="addTagOnEnter(event)" placeholder="태그를 입력하세요"> <select id="presetTags" name="presetTags"
